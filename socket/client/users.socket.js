        ...
        socket.on("CLIENT_ACCEPT_FRIEND", async (data) => {
            try {
                ...
                if (!existFriend && existRequest) {
                    const roomId = generate.randomToken(10);
                    await ClientAccount.updateOne(
                        { _id: myID },
                        { $push: { friends: { user_id: data.userID, room_chat_id: roomId } }, $pull: { requestFriend: data.userID } }
                    );

                    await ClientAccount.updateOne(
                        { _id: data.userID },
                        { $push: { friends: { user_id: myID, room_chat_id: roomId } }, $pull: { sendFriend: myID } }
                    );

                    // Cập nhật trường friends và requestFriend của myUser mà không load lại toàn bộ
                    myUser.friends.push({ user_id: data.userID, room_chat_id: roomId });
